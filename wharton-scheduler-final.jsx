...
const PENN_STUDENTS = [
  ...
  { name: "Aishwarya Rameshkumar", email: "aishrk@wharton.upenn.edu" }
];
...
export default function WhartonScheduler() {
    ...
    if (isMonitoring && meetingInvited && !hasShownAcceptances) {
      ...
      setTimeout(() => {
        setAttendeeResponses({
          ...
          "aishrk@wharton.upenn.edu": 'declined'
        });
        ...
        setConversationHistory(prev => [...prev, 
          ...
          {
            role: 'system',
            content: `⚠️ **RESPONSE RECEIVED**\n\n**Aishwarya Rameshkumar** (aishrk@wharton.upenn.edu) has **DECLINED** the meeting invite.\n\n🔄 Analyzing alternative times...`
          }
        ]);
  ...
  const findAlternativeTime = async () => {
    ...
    try {
      ...
      const prompt = `You are a scheduling assistant. Aishwarya has declined the meeting, but Gayatri and Manan accepted.
...
Find ONE alternative time that works for all 3 people including Aishwarya.
...
This new time works for all 3 attendees including Aishwarya.
...
✅ All 3 attendees (Gayatri, Manan, and Aishwarya) have been notified and the new time works for everyone.
...
❌ Aishwarya Rameshkumar will be marked as declined.
